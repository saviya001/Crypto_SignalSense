...
# Page configuration
st.set_page_config(
    page_title="Crypto SignalSense",
    page_icon="⚡",
    layout="wide",
    initial_sidebar_state="expanded"
)

# Custom Styling (Google Fonts + Clean Dark Theme)
st.markdown("""
<style>
...
<div style="text-align: center; padding: 15px 0 10px 0;">
    ...
    <p style="font-size: 15px; color: #94a3b8; max-width: 700px; margin: 4px auto 0 auto;">
        Cryptocurrency Trading Signal System powered by Indicators, News, and Strategy Knowledge.
    </p>
</div>
""", unsafe_allow_html=True)

# Academic & Educational Disclaimer Banner
st.warning("⚠️ **Academic & Educational Disclaimer:** Signals, Entry, TP, and SL estimates generated by this system are educational technical calculations derived from Support/Resistance levels and ATR volatility.")
...
# Real Crypto Dashboard Top Header Metrics
...
with col_action[1]:
    analyze_clicked = st.button("🚀 RUN MARKET ANALYSIS", type="primary", use_container_width=True)
...
if analyze_clicked or "last_result" in st.session_state:
    if analyze_clicked:
        if not groq_api_key or not openrouter_api_key:
            st.error("⚠️ **API Keys Required!** Both `GROQ_API_KEY` and `OPENROUTER_API_KEY` are mandatory to execute the market analysis. Please enter your API keys in the sidebar or configure `.streamlit/secrets.toml`.")
            st.stop()
        
        with st.spinner(f"Processing {selected_coin} market data..."):
